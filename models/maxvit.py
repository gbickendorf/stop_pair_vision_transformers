"""Base implementation from https://github.com/pytorch/vision/blob/main/torchvision/models/maxvit.py"""
import math
    ...
    def __init__(
        ...
        self.stem = nn.Sequential(
            Conv2dNormActivation(
                input_channels, stem_channels, 3, stride=2, norm_layer=norm_layer, activation_layer=activation_layer, bias=False,
                inplace=None,
            ),
            Conv2dNormActivation(
...
class MaxVit_mass(nn.Module):
    ...
    def __init__(
        ...
        self.head = nn.Sequential(
            nn.Linear(block_channels[-1]+1, 256),
            nn.ReLU(),
            nn.Linear(256, 128),
            nn.ReLU(),
...
if __name__ == '__main__':
    ...
    #mb=MBConv(10,18,4,0.5,1,nn.ReLU6,)
    i=6
    img = torch.randn(1, 3, 2**i, 2**i)
    print(img.shape)
    in_channels = 3
    num_blocks = [2, 2, 2, 2, 2]
    channels = [3, 96, 192, 384, 768]
    num_classes = 2
    net = MaxVitpt_info(input_size=(2**i,2**i),stem_channels=64, partition_size=4, block_channels=[64, 128, 256],
                              block_layers=[2, 2, 2],
                              head_dim=32,
                              stochastic_depth_prob=0.0, num_classes=2)
    out = net((img,torch.tensor([1])))#.to("cuda"))
    quit() 